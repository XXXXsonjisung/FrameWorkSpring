...
public class MemberController {
	...
	public String login(Member inputMember, Model model,
		...
		if(loginMember != null) { // 로그인 성공시
			 ...
			 * 
			 * - 쿠키파일에 등록됨 주소 요청 시 마다
			 *   자동으로 요청에 첨부되어있는 서버로 전달됨.
			 *   
			 * - 서버로 전달된 쿠키에
			 *   값 추가, 수정 삭제 등을 진행한 후
			 *   다시 클라이언트에게 반환
			...
			if(saveId != null) { // 체크가 되었을 때
				...
				// 0초 동안 유지되는 쿠키 생성
				// -> 기존에 쿠키가 지정되어있었다면 해당 쿠키를 삭제
				cookie.setMaxAge(0);
				
			}
			
			
			// 클라이언트가 어떤 요청을 할때 쿠키가 첨부될지 경로(주소) 지정
			 ...
			 * 
			 * RedirectAttribues를 스프링에서 제공
			 * - 리다이렉트 시 데이터를 request scope로 전달할 수 있게한느 객체
			 * 
			 * 응답 전 : request scope
			 * 
